...
// The omajot main window: sources (all notes, folders, tags, trash) │ notes │
// the editor with its preview. A normal Hyprland window, summoned by the host
// (`omarchy-shell` panel kind) from the bar, IPC, or a keybinding.
Item {
  id: root
  ...
  readonly property real editorFontSize: service && service.editorFontSize > 0
  ...
  FloatingWindow {
    ...
    FocusScope {
      ...
      Keys.onPressed: function(event) {
        ...
        else if (text === "q") root.requestClose()
        else return
        event.accepted = true
      }
        ...
        Item {
          ...
          ListView {
            ...
            delegate: NoteRow {
              ...
              accent: root.accent
              fontFamily: root.fontFamily
              border.width: selected && root.activePane === 1 && !focusScope.typing ? 1 : 0
              border.color: Style.focusBorderFor(root.foreground, root.accent)
              onClicked: {
                root.activePane = 1
